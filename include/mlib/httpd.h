...
inline const char* http_connection::get_uri () const
{
  return uri;
};

/// Return HTTP method (GET, POST, etc.)
inline const char* http_connection::get_method () const
{
  return request;
};

/// Return all incoming headers
inline const char* http_connection::get_all_ihdr () const
{
  return headers;
};

/// Return URI query string (everything after '?' and before '#')
inline const char* http_connection::get_query () const
{
  return query ? query : "";
};

/// Return request body
inline const char* http_connection::get_body () const
{
  return body;
};

/// Return request body
inline int http_connection::get_content_length () const
{
  return content_len;
};

/// Return socket object associated with this connection
inline sockstream& http_connection::out ()
{
  return ws;
};

/// Return current file origin
inline const char* httpd::docroot () const
{
  return root.c_str ();
}

/// Acquire lock on server's variables
inline void httpd::aquire_varlock ()
{
  varlock.enter ();
}

/// Release lock on server's variables
inline void httpd::release_varlock ()
{
  varlock.leave ();
}
...
*/
inline bool httpd::try_varlock ()
{
  return varlock.try_enter ();
}

inline httpd::handle_info::handle_info (uri_handler h_, void* info_)
  : h (h_)
  , nfo (info_)
