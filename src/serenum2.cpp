...
bool SerEnum_UsingSetupAPI (std::vector<int>& ports, std::vector<std::string>& names)
{
  ...
  HDEVINFO hDevInfoSet = SetupDiGetClassDevs (&GUID_DEVINTERFACE_COMPORT, nullptr, nullptr,
  ...
  while (more_items)
  {
    ...
    if (more_items)
    {
      ...
      HKEY device_key;
      device_key = SetupDiOpenDevRegKey (hDevInfoSet, &devInfo, DICS_FLAG_GLOBAL, 0, DIREG_DEV,
                                         KEY_QUERY_VALUE);
      if (device_key != INVALID_HANDLE_VALUE)
      {
        ...
        if (RegQueryValueEx (device_key, L"PortName", nullptr, &type, (BYTE*)port_name, &size)
              == ERROR_SUCCESS
            ...
            && (swscanf (port_name, L"COM%d", &port) == 1))
        {
          ...
          SetupDiGetDeviceRegistryProperty (hDevInfoSet, &devInfo, SPDRP_FRIENDLYNAME, &type,
                                            nullptr, 0, &size);
          wstring friendly (size, L'\0');
          if (SetupDiGetDeviceRegistryProperty (hDevInfoSet, &devInfo, SPDRP_FRIENDLYNAME, &type,
                                                (BYTE*)&friendly[0], size, &size)
              && (type == REG_SZ))
