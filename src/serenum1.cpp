...
/*!
  ...
  These functions are heavily inspired from [CEnumerateSerial]
  (http://www.naughter.com/enumser.html) code.
  ...
  for (int i = 1; i < 256; i++)
  {
    ...
    if (h == INVALID_HANDLE_VALUE)
    {
      ...
      if (error == ERROR_ACCESS_DENIED || error == ERROR_GEN_FAILURE
          || error == ERROR_SHARING_VIOLATION || error == ERROR_SEM_TIMEOUT)
        ok = true;
    }
    else
      ok = true;
